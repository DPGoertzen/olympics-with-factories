...
angular.module('olympicApp').controller('ArcheryController', function($scope){
  // instead make a get call.
  $scope.olympian = archerArray[random(archerArray.length - 1)];

});

angular.module('olympicApp').controller('BoxingController', function($scope){
  $scope.olympian = boxerArray[random(boxerArray.length - 1)];
});

angular.module('olympicApp').controller('PingpongController', function($scope){
  $scope.olympian = pingpongArray[random(pingpongArray.length - 1)];
});

angular.module('olympicApp').controller('TriathlonController', function($scope){
  $scope.olympian = triathleteArray[random(triathleteArray.length - 1)];
});
angular.module('olympicApp').controller('WeightliftingController', function($scope){
  $scope.olympian = weightlifterArray[random(weightlifterArray.length - 1)];
});
